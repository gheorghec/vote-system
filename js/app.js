...
function validate(a){
	...
	switch(a.name){
		case 'nume':
			if(a.value.match(/^[a-zA-Z\-]+$/) && a.value.length > 3){
				document.getElementById("nume").style.backgroundColor = "green";
			return true;
			}else{ 
		...
		case 'prenume':
			if(a.value.match(/^[a-zA-Z\-]+$/) && a.value.length > 3){
				document.getElementById("prenume").style.backgroundColor = "green";
			return true;
			}else{
				document.getElementById("prenume").style.backgroundColor = "red";
			}
		break;
		case 'cnp':
			if(a.value.length == 13){
				document.getElementById("cnp").style.backgroundColor = "green";
			return true;
			} else{
				document.getElementById("cnp").style.backgroundColor = "red";
			}
			break;
			}
...
function sbm(){
	if(validate(document.getElementById('nume'))
	&&validate(document.getElementById('prenume'))
	&&validate(document.getElementById('cnp'))){
		    $.getJSON( "vote.json", { get_param: 'value' }, function( data ) {
				  var items = [];
				  $.each( data, function( key, val ) {
				    var inp_cnp = document.getElementById('cnp').value;
				    if(inp_cnp == val.cnp){
